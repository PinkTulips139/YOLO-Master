        ...
        aggregate.update(
            {
                ...
                "precision_std": float(np.std([float(row["precision"]) for row in selected], ddof=1)),
                "recall_std": float(np.std([float(row["recall"]) for row in selected], ddof=1)),
                "mAP50_std": float(np.std([float(row["mAP50"]) for row in selected], ddof=1)),
                "mAP50_95_std": float(metrics.std(ddof=1)),
                "best_single_mAP50": float(max(float(row["mAP50"]) for row in selected)),
