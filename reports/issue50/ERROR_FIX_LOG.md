...
- **状态**：已验证 multiplier=0.2 弱于 0.1；正式配置冻结为 `lora_lr_mult=0.1`。
...
- **错误**：本地缺少 `cv2`；云端 `tests/test_engine.py` 又阻塞于从 GitHub 下载 `coco8.zip`。
- **原因**：测试依赖/数据不可用，并非断言失败。
- **处理**：停止精确 pytest/curl 进程，不让网络下载占用实例；语法检查通过，八组真实训练的退出码、产物和数值门控全部通过。
- **防复发**：离线回归前预置受信任的 coco8 测试数据与开发依赖。

# E17 VisDrone 公平基线显存不足

- **错误**：`visdrone_head_only_seed0` 在严格匹配 `batch=8, imgsz=768` 的第 7 个 epoch 前触发 `torch.OutOfMemoryError`，退出码 1。
- **根因**：冻结参数仍需保存检测头训练所需的中间激活，峰值已达 23.0 GiB；24 GiB RTX 4090 仅余约 60 MiB，无法再分配 96 MiB。
- **处理**：保留 6 个 epoch、日志、权重和失败 manifest；停止更高显存的 VisDrone full fine-tune，不将部分指标纳入正式比较。
- **防复发**：未来在预注册协议中统一使用能容纳所有方法的 batch，或采用梯度累积并重新完整运行所有对照；不得只为单个方法降低 batch。
